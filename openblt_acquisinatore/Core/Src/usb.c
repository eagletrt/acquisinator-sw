...
blt_bool UsbReceivePacket(blt_int8u *data, blt_int8u *len) {
    ...
    if (xcpCtoRxInProgress == BLT_FALSE) {
        ...
        if (UsbReceiveByte(&xcpCtoReqPacket[0]) == BLT_TRUE) {
            if ((xcpCtoReqPacket[0] > 0) && (xcpCtoReqPacket[0] <= BOOT_COM_USB_RX_MAX_DATA)) {
...
void UsbTransmitPipeBulkIN(void) {
    ...
    USBD_LL_Transmit(&hUsbDeviceFS, BULK_IN_EP, &USB_Tx_Buffer[0], nr_of_bytes_for_tx_endpoint);
...
void UsbReceivePipeBulkOUT(blt_int8u epnum) {
   ...
   */
    ...
    USBD_LL_PrepareReceive(&hUsbDeviceFS, BULK_OUT_EP, USBD_Bulk_GetRxBufferPtr(), BULK_DATA_FS_OUT_PACKET_SIZE);
